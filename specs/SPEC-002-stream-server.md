...
| `RQ-STRM-004` | A request from an IP that is not in `allowed_peers` MUST be refused with `403` and MUST NOT reach the audio source. | `integration` | `crates/rincon-stream/src/config.rs:155`<br>`crates/rincon-stream/src/config.rs:222`<br>`crates/rincon-stream/src/lib.rs:83`<br>`crates/rincon-stream/src/server.rs:269`<br>`docs/testing.md:118` |
