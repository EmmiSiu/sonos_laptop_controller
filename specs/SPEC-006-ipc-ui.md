...
| -- | ----------- | ------------ | ---------- |
| `RQ-UI-001` | Commands MUST accept a device **identifier**, never a raw address; the Rust side resolves it against known devices. | `unit` | `crates/rincon-ipc/src/lib.rs:284`<br>`crates/rincon-ipc/src/lib.rs:462` |
| `RQ-UI-002` | An unknown device id MUST return a typed `IpcError::UnknownDevice`, never panic and never 500. | `unit` | `crates/rincon-ipc/src/lib.rs:82`<br>`crates/rincon-ipc/src/lib.rs:284`<br>`crates/rincon-ipc/src/lib.rs:482` |
| `RQ-UI-003` | Every IPC error MUST serialise as `{ code, message, retryable }` with a stable `code`. | `unit` | `crates/rincon-core/src/stream_url.rs:82`<br>`crates/rincon-ipc/src/lib.rs:40`<br>`crates/rincon-ipc/src/lib.rs:499` |
...
| `RQ-UI-006` | The latency characteristic MUST be visible in the connected state, not hidden behind a menu. | `manual` | `crates/rincon-ipc/src/lib.rs:155`<br>`crates/rincon-ipc/src/lib.rs:337`<br>`crates/rincon-ipc/src/lib.rs:564`<br>`docs/manual-test-log.md:18` |
| `RQ-UI-007` | On `FirewallSuspected`, the UI MUST show the exact remediation command and MUST NOT silently execute a privileged action. | `unit` | `crates/rincon-ipc/src/lib.rs:202`<br>`crates/rincon-ipc/src/lib.rs:356`<br>`crates/rincon-ipc/src/lib.rs:521` |
| `RQ-UI-008` | TypeScript bindings MUST be generated from the Rust DTOs; a mismatch MUST fail the frontend build. | `integration` | `crates/rincon-ipc/src/bindings.rs:165`<br>`.github/workflows/ci.yml:108` |
