...
| -- | ----------- | ------------ | ---------- |
| `RQ-AUD-001` | The producer path MUST NOT allocate, lock, or perform I/O once capture has started. | `unit` | `crates/rincon-audio/src/ring.rs:203`<br>`crates/rincon-audio/src/ring.rs:404` |
| `RQ-AUD-002` | The ring MUST preserve frame alignment: a partial frame MUST never be visible to the consumer. | `property` | `crates/rincon-audio/src/ring.rs:203`<br>`crates/rincon-audio/src/ring.rs:295`<br>`crates/rincon-audio/src/ring.rs:436` |
| `RQ-AUD-003` | On overrun the ring MUST discard the **oldest** whole frames and increment `frames_dropped` by exactly the number discarded. | `property` | `crates/rincon-audio/src/ring.rs:203`<br>`crates/rincon-audio/src/ring.rs:456` |
...
| `RQ-AUD-008` | Dropping a `CaptureSession` MUST stop the underlying stream and MUST NOT leak the OS handle. | `integration` | `crates/rincon-audio/src/lib.rs:271`<br>`crates/rincon-audio/src/loopback.rs:285` |
| `RQ-AUD-009` | Ring capacity MUST be configurable and MUST default to at least 500 ms of audio at the negotiated format. | `unit` | `crates/rincon-audio/src/ring.rs:162`<br>`crates/rincon-audio/src/ring.rs:500` |
| `RQ-AUD-010` | When the device disappears, the consumer MUST observe a terminal `DeviceLost` state rather than blocking forever or silently returning silence. | `unit` | `crates/rincon-audio/src/ring.rs:253`<br>`crates/rincon-audio/src/ring.rs:295`<br>`crates/rincon-audio/src/ring.rs:518` |
| `RQ-AUD-011` | Interleaved multi-channel frames MUST round-trip through the ring in channel order. | `property` | `crates/rincon-audio/src/ring.rs:203`<br>`crates/rincon-audio/src/ring.rs:295`<br>`crates/rincon-audio/src/ring.rs:536` |
