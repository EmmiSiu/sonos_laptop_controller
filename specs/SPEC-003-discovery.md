...
| `RQ-DISC-009` | Duplicate responses for the same UDN MUST collapse to a single `Device`. | `unit` | `crates/rincon-discovery/src/scan.rs:130`<br>`crates/rincon-discovery/src/scan.rs:372` |
...
| `RQ-DISC-011` | Discovery MUST return within `timeout + 500 ms` even when no device answers. | `integration` | `crates/rincon-discovery/src/scan.rs:130`<br>`crates/rincon-discovery/src/scan.rs:355` |
