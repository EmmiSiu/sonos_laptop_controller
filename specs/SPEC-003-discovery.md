...
| `RQ-DISC-002` | Header parsing MUST be case-insensitive for names and MUST tolerate `\n` as well as `\r\n` line endings. | `property` | `crates/rincon-discovery/src/ssdp.rs:105`<br>`crates/rincon-discovery/src/ssdp.rs:154`<br>`crates/rincon-discovery/src/ssdp.rs:243` |
...
| `RQ-DISC-005` | The description fetch MUST NOT follow redirects. | `integration` | `crates/rincon-discovery/src/fetch.rs:58`<br>`crates/rincon-discovery/src/fetch.rs:76`<br>`crates/rincon-discovery/src/fetch.rs:167` |
| `RQ-DISC-006` | The description response body MUST be capped at 256 KiB; a larger body MUST abort the fetch. | `integration` | `crates/rincon-discovery/src/fetch.rs:76`<br>`crates/rincon-discovery/src/fetch.rs:190`<br>`crates/rincon-testkit/src/mock_sonos.rs:363` |
| `RQ-DISC-007` | XML parsing MUST reject documents containing a DOCTYPE or entity declaration (XXE / billion-laughs guard). | `unit` | `crates/rincon-core/src/xml.rs:46`<br>`crates/rincon-core/src/xml.rs:132`<br>`crates/rincon-discovery/src/description.rs:85`<br>`crates/rincon-discovery/src/description.rs:281`<br>`crates/rincon-testkit/src/mock_sonos.rs:363` |
| `RQ-DISC-008` | Parsing MUST NOT panic on any input. Malformed input yields `Err`. | `fuzz` | `crates/rincon-discovery/src/description.rs:85`<br>`crates/rincon-discovery/src/description.rs:293`<br>`crates/rincon-discovery/src/ssdp.rs:154`<br>`crates/rincon-discovery/src/ssdp.rs:274` |
| `RQ-DISC-009` | Duplicate responses for the same UDN MUST collapse to a single `Device`. | `unit` | `crates/rincon-discovery/src/scan.rs:215`<br>`crates/rincon-discovery/src/scan.rs:569` |
| `RQ-DISC-010` | Discovery MUST probe every non-loopback IPv4 interface, and MUST record which local address reached each device. | `integration` | `crates/rincon-discovery/src/scan.rs:215`<br>`crates/rincon-discovery/src/scan.rs:458` |
| `RQ-DISC-011` | Discovery MUST return within `timeout + 500 ms` even when no device answers. | `integration` | `crates/rincon-discovery/src/scan.rs:215`<br>`crates/rincon-discovery/src/scan.rs:526`<br>`crates/rincon-discovery/src/scan.rs:552` |
| `RQ-DISC-012` | A room name containing XML-escaped or control characters MUST be decoded and sanitised before reaching the UI. | `property` | `crates/rincon-core/src/device.rs:102`<br>`crates/rincon-core/src/device.rs:235`<br>`crates/rincon-discovery/src/description.rs:85`<br>`crates/rincon-discovery/src/description.rs:314` |
