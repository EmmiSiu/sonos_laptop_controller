...
| `RQ-DISC-009` | Duplicate responses for the same UDN MUST collapse to a single `Device`. | `unit` | `crates/rincon-discovery/src/scan.rs:215`<br>`crates/rincon-discovery/src/scan.rs:525` |
| `RQ-DISC-010` | Discovery MUST probe every non-loopback IPv4 interface, and MUST record which local address reached each device. | `integration` | `crates/rincon-discovery/src/scan.rs:215`<br>`crates/rincon-discovery/src/scan.rs:432` |
| `RQ-DISC-011` | Discovery MUST return within `timeout + 500 ms` even when no device answers. | `integration` | `crates/rincon-discovery/src/scan.rs:215`<br>`crates/rincon-discovery/src/scan.rs:508` |
